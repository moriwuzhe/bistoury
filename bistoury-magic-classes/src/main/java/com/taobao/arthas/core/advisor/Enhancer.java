...
import qunar.tc.bistoury.arthas.common.Pair;
import qunar.tc.bistoury.arthas.core.GlobalOptions;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyInterceptor1;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyInterceptor2;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyInterceptor3;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyTraceExcludeJDKInterceptor1;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyTraceExcludeJDKInterceptor2;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyTraceExcludeJDKInterceptor3;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyTraceInterceptor1;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyTraceInterceptor2;
import qunar.tc.bistoury.arthas.core.advisor.SpyInterceptors.SpyTraceInterceptor3;
import qunar.tc.bistoury.arthas.core.server.ArthasBootstrap;
import qunar.tc.bistoury.arthas.core.advisor.TransformerManager;
import qunar.tc.bistoury.arthas.core.util.ArthasCheckUtils;
import qunar.tc.bistoury.arthas.core.util.ClassUtils;
import qunar.tc.bistoury.arthas.core.util.FileUtils;
import qunar.tc.bistoury.arthas.core.util.SearchUtils;
import qunar.tc.bistoury.arthas.core.util.affect.EnhancerAffect;
import qunar.tc.bistoury.arthas.core.util.matcher.Matcher;
...
public class Enhancer implements ClassFileTransformer {
    ...
    // 被增强的类的缓存
    public static final Map<Class<?>, byte[]> classBytesCache = new WeakHashMap<Class<?>, byte[]>();
    private static Object spyImpl = new SpyImpl();
    public static final java.util.concurrent.locks.Lock lock = new java.util.concurrent.locks.ReentrantLock();

    static {
        // SpyAPI is in java.arthas package, it is injected at runtime by arthas agent, 
        // Java does not allow compiling custom java.* package, so we can't use direct reference
        // All access via reflection, keep it. We just need the string "java.arthas.SpyAPI"
        try {
            // spyAPI class is java.arthas.SpyAPI loaded at runtime
            Class<?> spyAPIClass = Class.forName("java.arthas.SpyAPI");
            // spy class is java.arthas.Spy loaded at runtime
            Class<?> spyClass = Class.forName("java.arthas.Spy");
            Method setSpyMethod = spyAPIClass.getMethod("setSpy", spyClass);
            setSpyMethod.invoke(null, spyImpl);
        } catch (Exception e) {
            logger.error("init Spy failed, this should not happen at runtime", e);
        }
    }
        ...
        try {
            ...
            try {
                if (inClassLoader != null) {
                    inClassLoader.loadClass("java.arthas.SpyAPI");
                }
            } catch (Throwable e) {
            ...
            // remove JSR https://github.com/alibaba/arthas/issues/IC
            ...
            // SpyAPI is java.arthas.SpyAPI at runtime, internal name is "java/arthas/SpyAPI"
            // Because Java doesn't allow compiling custom java.* package, so we just hardcode it here
            String spyApiInternalName = "java/arthas/SpyAPI";

            GroupLocationFilter groupLocationFilter = new GroupLocationFilter();

            LocationFilter enterFilter = new InvokeContainLocationFilter(spyApiInternalName, "atEnter",
                    LocationType.ENTER);
            LocationFilter existFilter = new InvokeContainLocationFilter(spyApiInternalName, "atExit",
                    LocationType.EXIT);
            LocationFilter exceptionFilter = new InvokeContainLocationFilter(spyApiInternalName,
            ...
            LocationFilter invokeBeforeFilter = new InvokeCheckLocationFilter(spyApiInternalName,
                    "atBeforeInvoke", LocationType.INVOKE);
            LocationFilter invokeAfterFilter = new InvokeCheckLocationFilter(spyApiInternalName,
                    "atInvokeException", LocationType.INVOKE_COMPLETED);
            LocationFilter invokeExceptionFilter = new InvokeCheckLocationFilter(spyApiInternalName,
            ...
            for (MethodNode methodNode : matchedMethods) {
                ...
                if(AsmUtils.containsMethodInsnNode(methodNode, spyApiInternalName, "atBeforeInvoke")) {
    ...
    private static Pair<Boolean, String> isUnsupportedClass(Class<?> clazz) {
        ...
        if (clazz.equals(java.lang.reflect.Method.class)) {
            return new Pair<Boolean, String>(Boolean.TRUE, "class is java.lang.reflect.Method");
        }
    ...
    public synchronized EnhancerAffect enhance(final Instrumentation inst, int maxNumOfMatchedClass) throws UnmodifiableClassException {
        ...
        try {
            ...
            // Note: arthas 3.x+ has removed addLazyTransformer, the new addTransformer already handled it
            if (isLazy) {
